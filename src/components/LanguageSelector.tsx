      ...
      <div className="absolute right-0 top-full mt-2 w-48 bg-slate-800 border border-slate-700 rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
        {languages.map((lang) => (
          <button
            ...
            className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-slate-700 transition first:rounded-t-lg last:rounded-b-lg ${
              language === lang.code ? 'bg-slate-700 text-orange-400' : 'text-white'
            }`}
          >
